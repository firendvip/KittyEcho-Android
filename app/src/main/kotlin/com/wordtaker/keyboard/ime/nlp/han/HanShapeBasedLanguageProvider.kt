...
package com.wordtaker.keyboard.ime.nlp.han
...
import com.wordtaker.keyboard.ime.nlp.LanguagePackComponent
import com.wordtaker.keyboard.ime.nlp.LanguagePackExtension
import com.wordtaker.keyboard.ime.nlp.SpellingProvider
import com.wordtaker.keyboard.ime.nlp.SpellingResult
...
class HanShapeBasedLanguageProvider(val context: Context) : SpellingProvider, SuggestionProvider {
    ...
    private val activeLanguagePacks  // language packs referenced in subtypes
        get() = buildSet {
            // Include the locales of the enabled subtype list AND the currently-active subtype.
            // The active subtype matters because on a fresh install the persisted subtype list can
            // still be empty while the keyboard already runs on the style-aware fallback subtype
            // (e.g. zh-CN-wubi / zh-CN-stroke). Without the active subtype here, the shape-based
            // language pack would never be loaded and every query would fail with "no such table".
            val locales = (subtypeManager.subtypes.map { it.primaryLocale.localeTag() } +
                subtypeManager.activeSubtype.primaryLocale.localeTag()).toSet()
