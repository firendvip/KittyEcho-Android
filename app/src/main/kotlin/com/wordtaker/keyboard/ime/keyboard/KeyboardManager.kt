...
import com.wordtaker.keyboard.lib.titlecase
import com.wordtaker.keyboard.lib.uppercase
...
import com.wordtaker.lib.android.AndroidKeyguardManager
import com.wordtaker.lib.android.showLongToast
import com.wordtaker.lib.android.showLongToastSync
import com.wordtaker.lib.android.showShortToastSync
import com.wordtaker.lib.android.systemService
import com.wordtaker.lib.kotlin.collectIn
...
class KeyboardManager(context: Context) : InputKeyEventReceiver {
    ...
    fun executeSwipeAction(swipeAction: SwipeAction) {
        // WordTaker: space long-press switches to the cat voice界面 (a parallel main
        // surface) instead of any IME-picker / subtype switch. Handled before the
        // key-data mapping below.
        if (swipeAction == SwipeAction.TRIGGER_VOICE_INPUT) {
            // 单界面 (item6): start recording on the merged panel rather than switching屏.
            if (activeState.imeUiMode != ImeUiMode.TEXT && activeState.imeUiMode != ImeUiMode.CAT_VOICE) {
    ...
    /**
     * Handles a [KeyCode.LANGUAGE_SWITCH] event. WordTaker repurposes this key as an
     * in-keyboard 中/英 toggle: Chinese mode runs the pinyin candidate pipeline, English
     * mode commits latin letters directly (no Chinese candidates). Toggling always
    ...
    private fun handleSpace(data: KeyData) {
        // In Chinese pinyin mode: if there is an active composing region, commit the first
        // Hanzi candidate (or the raw pinyin if no candidates) instead of inserting a space.
        val isPinyinComposing = subtypeManager.activeSubtype.primaryLocale.language.startsWith("zh") &&
            editorInstance.activeContent.composing.length > 0
        if (isPinyinComposing) {
    ...
    override fun onInputKeyUp(data: KeyData) = activeState.batchEdit {
        ...
        when (data.code) {
            ...
            KeyCode.IME_UI_MODE_CAT_VOICE -> {
                // 单界面 (item6): stay on the merged keyboard/cat panel and start recording.
                if (activeState.imeUiMode != ImeUiMode.TEXT && activeState.imeUiMode != ImeUiMode.CAT_VOICE) {
                    activeState.imeUiMode = ImeUiMode.TEXT
                }
                com.wordtaker.keyboard.wordtaker.voice.VoiceTrigger.requestStart()
    ...
    private inner class ComputingEvaluatorImpl(
        ...
        override fun evaluateEnabled(data: KeyData): Boolean {
            return when (data.code) {
                ...
                KeyCode.LANGUAGE_SWITCH -> {
                    // WordTaker repurposes this as the 中/英 toggle: always actionable,
                    // regardless of how many subtypes are installed.
                    true
                }
                else -> true
            }
        ...
        override fun evaluateVisible(data: KeyData): Boolean {
            return when (data.code) {
                ...
                KeyCode.LANGUAGE_SWITCH -> {
                    // WordTaker 中/英 toggle is always shown on the bottom row.
                    true
                }
                else -> true
            }
