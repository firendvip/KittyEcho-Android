...
class EditorInstance(context: Context) : AbstractEditorInstance(context) {
    ...
    override fun handleStartInputView(editorInfo: FlorisEditorInfo, isRestart: Boolean) {
        ...
        activeState.isComposingEnabled = when (keyboardMode) {
            ...
            KeyboardMode.PHONE2,
            -> false
            // WordTaker: in 英文 mode latin keys must commit directly without
            // forming a pinyin composing region or producing Chinese candidates.
            else -> !activeState.isEnglishMode &&
                activeState.keyVariation != KeyVariation.PASSWORD &&
                // Honor providers that force suggestions on (e.g. the Pinyin CJK
                // provider). Without this, when the global suggestion toggle is off
                // (its default), the composing region never forms, so pinyin keys
                // commit as raw latin and no candidates are ever produced.
