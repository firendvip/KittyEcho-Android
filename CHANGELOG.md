...
### Added
- **本地 Paraformer 语音识别**：首次使用时下载并校验固定版本的 Paraformer INT8 模型与词表，安装到应用私有目录后可完全离线识别；设置页新增「仅本地识别」开关、模型状态、下载失败处理及模型/运行时许可证说明。
- **可复现 ASR 评测工具链**：新增隔离的盲测、准确率、性能、稳定性、制品哈希与 Android 签名证据合同；真实录音、模型和评测结果始终保留在仓库外。

### Changed
- **第一层语音识别基座更新**：默认从旧 Zipformer 流式识别切换为整句 Paraformer INT8，停止录音后先释放麦克风，再由单例 FIFO actor 解码；下一段录音无需等待上一段识别完成。
- **模型交付与隐私边界收紧**：APK 不再内置语音模型，模型只从固定来源下载并按字节数与 SHA-256 校验；密码、支付、验证码、隐身及禁止个性化学习场景仅本地处理且不写历史，输入框切换后旧录音不会提交到新输入框。

### Fixed
- **录音与识别失败显式处理**：麦克风权限、录音启动、采集读取、模型缺失或损坏、初始化、内存不足及解码失败均返回明确状态，不再伪装为正在录音或静默空文本。

## [0.41.2] - 2026-07-29

### Fixed
- **多音节全拼分段选择与可逆退格**：全拼候选现在可在同一 composing 会话中按音节逐段选择，例如 `nihao → 你hao → 你好`，仅在全部音节选完时提交一次，完整词组仍可直接提交；本地与云候选在展开、收起和异步重排后均使用各自的分段快照映射。混合预编辑按 Enter 只提交一次原始拼音且不附加换行，首次退格撤销最近一次分段选择、再次退格恢复普通逐字删除；陈旧、空白、越界或异常长度候选均安全拒绝而不吞字。

## [0.41.1] - 2026-07-28

### Fixed
- **录音首字完整保留与反馈时序**：删除固定 850ms 句首 PCM 丢弃及开麦后的开始喵叫，录音从首帧开始完整送入本地识别，并在采集启动后仅按既有键盘震动开关触发短震；正常结束、取消与异常回退均先冻结或关闭 PCM，再按提示音设置播放结束喵叫，震动或提示音异常不阻断识别与提交。

## [0.41.0] - 2026-07-28

### Changed
- **候选原始拼音持续可见**：候选异步刷新、重排或云结果为空时，顶栏继续以编辑器 composing 为唯一真源显示原始拼音，直到用户选词或执行编辑动作；单字与词组选中白底统一为 30dp 外框，触控层仍覆盖完整候选栏。
- **润色阶段小猫单独放大**：仅 `VoicePhase.Polishing` 在宽屏使用 46×57dp 小猫与 62dp 视口、窄屏使用 23×40dp 小猫与 39dp 视口，左右各保留 8dp 运动区；其它语音阶段尺寸不变。
- **高情商改写提示词上线**：广州 `wordtaker-relay-web` 的 `$LATEST` 已更新高情商角色提示词；Android 客户端仍不内置、下载或保存提示词，请求与失败回退边界不变。

### Fixed
- **换行键提交原始拼音**：显示换行键且存在 composing/candidates 时只提交一次原始拼音并吞掉本次换行；无 composing 时正常换行，Done、Search、Send 等非换行 editor action 仍在提交 raw 后执行原动作。

### Notes
- **云词库仅完成诊断**：固定合成全拼请求的生产接口返回 HTTP 201（约 141–246ms）；当前可见效果受多重门禁、仅接受精确 ASCII 全拼、jieba 覆盖稀疏及与本地候选去重后无来源标识共同影响。本版本未扩展云词库或候选交互，不将其表述为已修复。

## [0.40.0] - 2026-07-27

### Added
- **统一键盘震动开关**：主设置页、输入法内设置和备用设置页均可查看并修改同一持久化开关；新用户默认开启，且与按键声音相互独立。
- **历史润色归因**：新历史记录明确标注「云端AI」或「未润色」，旧记录保持未知归因，不猜测来源。
- **单条历史删除确认**：两个历史入口删除单条记录前均显示绑定该记录摘要的确认框；取消、返回、点外和重复点击不会误删其他记录。

### Changed
- **产品键盘仅保留全拼与九宫格**：产品选择、引导和深链入口隐藏手写样式，已有手写配置幂等迁移为全拼，底层识别资产保留。
- **候选选中高度稳定**：单字和词组选中白底保持同高，候选栏及键盘顶边不再随候选长度跳动。
- **润色阶段小猫加宽**：仅润色阶段使用 42×52dp 小猫视口并保留左右安全间距，其他语音阶段和说话按钮触控区域不变。
- **中/英切换视觉重排**：激活语言使用大字、非激活语言使用小字对角排布，按键宽度和极端字体缩放下仍保持布局边界。
- **设置态顶栏统一**：文字页与设置页复用左侧「猫头像＋点击说话」组合；设置态点击说话会退出设置并开始录音，独立右侧语音按钮移除，收起键保留。
- **空格键复用说话图标**：短按仍输入空格，长按进入录音，滑动与候选提交路径保持不变。

### Fixed
- **拼音合成态回车语义**：按回车先且仅提交一次原始拼音，再执行当前编辑器动作或换行；空格仍提交首候选。

## [0.39.0] - 2026-07-27

### Added
- **账号资料状态与仓库内水合**：账号明确区分退出、加载中、可用和暂不可用；登录或读取持久化凭据后立即拉取 `/auth/me`，失败可重试且界面不再出现 `null`，资料暂不可用不门禁匿名或有效设备身份的长文本润色。
- **连续语音反馈与统一小猫头像**：录音、识别、润色、成功及后台段数使用同一顶栏状态链；结束一段后可立即开始下一段并保持 FIFO，上方两个设置入口统一为 44dp 触控区、34dp 背景和 30dp 小猫头像。

### Changed
- **短文本与提示音默认值**：1–6 个可见扩展字素簇严格原文直出，7 个及以上仅在经验证联网时每段发起一次顶层润色；新用户提示音默认 30%，既有用户已保存值不迁移。
- **睡眠反馈直接切换**：静音判定后小猫立即进入睡眠，不再经过 800ms settle、缩小或残影；8/10/12sp 的 Z 保持原倍率并移至趴睡猫脸左上方。
- **原生微信登录边界保持不变**：本版本未实现 Android 微信 OpenSDK 原生登录；现有网页登录兼容流程不作为原生微信登录交付。

### Fixed
- **认证、诊断与凭据安全**：资料水合、退出和 401 失效使用会话代数与同步边界，避免旧请求覆盖新状态；润色失败仅记录不含正文、token、业务码或服务端消息的固定分类；Android Keystore 不可用时 token 持久化 fail-closed，历史明文迁移失败即删除。
- **中/英切换位冲突与候选提交**：英文状态位移出 IME UI mode 位域；短按始终回到文字页并双向切换，有拼音候选时先提交当前首项，否则提交原 composing，长按仍打开系统输入法选择器。
- **窄屏顶栏与后台段状态**：非录音顶栏固定为“设置猫 → 点击说话 → 状态 → ×N → 弹性空白 → 收起”，在窄屏优先压缩装饰文本而保留操作语义和触控目标。

## [0.38.0] - 2026-07-26

### Added
- **全套品牌应用图标**：以指定白底小猫 SVG 为唯一真源，统一普通、圆形、自适应与主题单色图标，并覆盖启动器、应用信息、输入法选择器及启动页。
- **CatSkin 全流程语音反馈**：录音、转录、在线润色和成功分别呈现小猫状态，录音动画读取真实 PCM 音量，并遵循系统减动效与无障碍语义。
- **符号页 8 分类标签（全部真实可用）**：「符号」页底行改为 [←][时钟][中][EN][[]][¥][数][(-)][①] 分类标签；点分类即整体切换第 1–3 行的 26 个符号槽位（10+10+6），底行不变，选中分类绿色高亮。时钟=最近使用（本地 MRU 持久化 `files/wt_sym2_recent.txt`，从符号/数字页上屏的符号实时记录，不足 26 个用常用符号补齐，进入页面/切分类时才刷新快照、打字中不洗牌）；中=中文标点、EN=英文符号、[]=括号、¥=货币、数=数学、(-)=破折/连接号、①=带圈序号。
...
### Changed
- **润色仅限经验证的联网路径**：客户端不内置、下载或保存润色提示词；云端请求只发送 `text`、`mode` 与既有可选 `word_map`。离线、失败及不超过 6 个可见扩展字素簇的转录原文直出，7 个及以上才按联网策略润色，FIFO/历史链路保持不变。
- **全拼云候选按分数优先**：云候选稳定按 score 排序并以 NFC 去重；仅在经验证网络、普通非密码/非隐身且允许个性化学习的全拼中文场景启用，不发送上下文，并防护触摸快照与乱序请求竞态。
- **手机竖屏键盘上移 8/360 屏宽**：仅 `PHONE_PORTRAIT + FIXED + NORMAL` 增加底部人体工学间隙，其他形态和窗口模式不变。
- **三个符号/数字页按参考图重做（总高与字母键盘像素恒等）**：①「123」页：数字行 + 常用符号行 + [符号|。，、？！.|⌫] + [←|12·34|空格|换行]；②「12·34」页改为 iOS 风计算器网格——左列 % + - * 四枚运算键（4 键占 3 行高）、中间 3×3 数字、右列 ⌫ / . / 双倍高「换行」块、底行 [←|符号|0|空格]（TextKeyboard 新增专用几何布局）；③「符号」页三行符号 + 分类标签行。所有页均为顶栏+4 行、总高与字母键盘逐像素相等（480dpi 实测 6 页 panel top 全部 y=1409），切换零跳动；顶栏（点击说话条）现在所有键盘模式下都显示。原键区上方 38dp 快捷符号条（QuickSymbolStrip）随之移除。
- **←（回字母键）改绿色主题色**：符号/数字各页的返回字母键由灰色功能键改为微信绿主题色（与换行键同款，日/夜主题同步）。
- **键盘尺寸全面按实际屏幕比例（真机 1:1）**：手机竖屏下行距(0.1485×屏宽)、键水平/垂直间距、顶栏高(0.152×屏宽)、底部空隙、键盘总高及录音/设置/历史/手写面板高全部改为按实际屏幕宽度比例计算，不再依赖 395×875 基准屏固定 dp；键帽字号随键帽高同比缩放（字母 glyph≈0.40×键帽高、功能键≈0.31），任何设备/密度下键盘占屏比例一致（480/420dpi 双密度实测比例一致）。
- **顶栏小猫设置钮去白圈**：键盘顶条与设置面板工具栏的小猫头像钮去掉白色圆底，只留猫头图形（新增无底矢量 ic_brand_cat_bare），点击区域保持 44dp 不可见触控。
- **录音重做：点击才转写 + 多段并行（对齐 PC 端）**：录音中停顿（endpoint）不再定稿出字、也不显示实时字幕——引擎在后台流式解码攒段，界面只有小猫动画 +「正在倾听…点击结束」；点击结束时当前段（攒段+尾巴）整段定稿交给后台「润色→上屏」任务，界面立刻回到待机可马上开新一段，多段并行处理、按开始顺序 FIFO 上屏（润色入队即并发启动，worker 按序 await）。后台有 N 段在处理时，待机顶栏与录音界面显示「忙碌小猫 ×N」角标。取消=只丢弃当前录音段（队列中的段不受影响）；切 App / 收键盘=停当前录音，队列中的段继续完成上屏（守卫不变）。
...
### Fixed
- **增量构建 APK 完整性**：打包后保留 AGP 管理的 canonical APK，并覆盖复制同字节的版本化 branded APK，不再用 `renameTo` 移走 canonical 输出，连续增量构建仍可安装且 Manifest 完整。
...
- **App 设置页滚动 ANR + 设置写入丢失（P0）**：设置页（`MinimalSettingsScreen`）滚动时复现 `Input dispatching timed out … Waited 5052ms for FocusEvent`、主线程 CPU 125%、`Choreographer: Skipped 30–80 frames`，进程被系统强杀导致刚改的设置（云词库开关、提示音音量）丢失/回滚。ANR trace 的绘制调用栈定位到根因：设置主页此前是一个普通 `Column` + `Modifier.verticalScroll()`，滚动时 Compose 每帧都要测量/布局/绘制全部内容（包括屏幕外的关于区块二维码图片），而非仅可见部分；另外 `InputMethodUtils` 里 Android 14+ 专用的输入法状态轮询（`timedObserveIsFlorisBoardEnabled/Selected`）在一个 `while(true)` 循环里直接在主线程做同步 Binder IPC，每 500ms 一次、设置页开着就永不停止，一旦系统进程短暂卡顿即可拖垮主线程。现将设置主页由 `Column` 改为 `LazyColumn`（只测量/绘制可见区域），并把上述轮询查询移到 `Dispatchers.IO` 执行。真实环境复验（干净重启模拟器后）：设置 App 冷启动 3 次全部 826–985ms 可交互、0 ANR；此前必现卡顿的高强度滚动压测（40 组快速上下 swipe）0 次 `Skipped frames`、0 次 ANR；拨云词库开关+拖音量至 23% 后 `force-stop` 强杀进程、冷启动重开，两项设置均正确保留；键盘本体中文拼音打字（nihao→候选「你好」）冒烟正常，无回归。
- **App 设置页冷启动仍偶发 ANR（P0 收尾）**：独立复验发现上一版 `LazyColumn` 修复只解决了滚动期间的重复测量/绘制，未覆盖冷启动首帧——现场在冷启动重开场景再次复现与原 P0 一字不差的 `Waited 5002ms for FocusEvent` ANR + 进程被杀，ANR trace 主线程栈卡在设置页首次组合（`MinimalSettingsScreen → FlorisScreen → Scaffold`）。逐项排查代码路径（DataStore 读取、输入法状态轮询、图标渲染）后定位到一处真实可修的浪费：品牌头像图标 `FlorisCanvasIcon` 每次组合都无条件重新分配位图+软件画布光栅化绘制一次，未用 `remember` 缓存，现改为按 drawable 记忆一次结果。另经现场取证确认：本次复现所用的 `.debug` 调试包在 Android 侧被强制降级为 `verify`（无 AOT/无 profile-guided 编译，`dumpsys package` 显示 `run-from-apk`/`verify`，即便手动 `pm compile -m speed -f` 也无法绕过——这是系统对可调试应用的固定限制，非本次改动可控），导致 Compose/Material3 大量类在冷启动时以解释执行，这是该场景耗时偏高的主要环境因素，正式发布走 release/minify 签名包不受此限制。全新模拟器实例复验：设置 App 冷启动连续 6 次全部 773–991ms 可交互、0 ANR（`/data/anr` 0 新增、`Skipped ≥200 frames` 0 次）；40 次滚动压测 0 ANR；云词库开关+音量拖动后 `force-stop` 强杀重开，两项设置正确保留；键盘中文拼音打字（nihao→候选「你好」）冒烟正常。
